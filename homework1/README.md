...
## 5. 彩色影像壓縮 (RGB Extension)

### 5.1 原理說明
傳統 SVD 僅適用於二維矩陣，然而彩色影像通常由紅 (R)、綠 (G)、藍 (B) 三個通道組成，在程式中表示為 $H \times W \times 3$ 的三維張量 (Tensor)。若要對彩色影像進行壓縮，我們採用「分而治之」的策略：

1.  **通道分離 (Splitting)**：將彩色影像拆解為三個獨立的 $H \times W$ 矩陣（R矩陣, G矩陣, B矩陣）。
2.  **獨立壓縮 (Independent Compression)**：對這三個矩陣分別執行 SVD 分解與低秩近似。
3.  **影像合成 (Stacking)**：將壓縮還原後的三個矩陣重新堆疊，並將數值限制在 [0, 255] 區間，合成最終的彩色影像。

### 5.2 彩色壓縮結果
下圖展示了 RGB 模式下的壓縮成果。由於彩色影像包含更多資訊量，我們觀察到即便 $k=50$ (約 18% 壓縮率)，影像品質依然極高，幾乎無法分辨與原圖的差異。

![RGB壓縮結果](rgb_compression_result.png)

---

## 6. 結論 (Conclusion)

本專案成功展示了奇異值分解 (SVD) 在影像處理領域的實際應用。

1.  **數學理論的實踐**：我們驗證了抽象的線性代數理論（$A=U\Sigma V^T$）如何轉化為具體的工程應用。
2.  **高效的特徵提取**：實驗數據顯示，影像的主要能量高度集中在前 10% - 20% 的奇異值中。這意味著我們只需儲存極少量的數據，就能還原出高品質的視覺內容。
3.  **通用性**：從灰階到 RGB 彩色影像的實作，證明了此演算法具有良好的擴充性與通用性。
